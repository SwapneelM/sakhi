...
from csv_prep import prepare_for_english_generation
from paths import (
    LLM_ENGLISH_EXPERT_150,
    LLM_ENGLISH_NON_EXPERT_230,
    OUTPUTS,
    SAKHI_EXPERT_RAW_150,
    SAKHI_NON_EXPERT_RAW_230,
)
...
SAKHI_PROMPT = """You are a knowledgeable and caring assistant trained to support pregnancy-related health. Your task is to provide accurate, empathetic, and reliable answers to user questions specifically about pregnancy, prenatal care, and postnatal well-being.
...
class CohereLM(dspy.LM):
    def __init__(self, model_name: str, api_key: str):
        import cohere
        super().__init__(model=model_name, temperature=0.7, max_tokens=200)
...
class OpenRouterLM(dspy.LM):
    def __init__(self, model_name: str, api_key: str):
        ...
        self.max_tok = 16000 if ("gpt-5" in model_name.lower() or "gpt5" in model_name.lower()) else 200
    def __call__(self, prompt=None, messages=None, **kwargs):
        if messages is None:
            messages = [{"role": "user", "content": prompt}]
        extra_body = {}
        if "gemini" in self.model_name.lower():
            # Use low effort + exclude: models like gemini-2.5-pro require reasoning and cannot disable it
    ...
    def __call__(self, prompt=None, messages=None, **kwargs):
        ...
        for attempt in range(5):
            try:
                ...
                response = self.session.post(self.endpoint_url, headers={"Authorization": f"Bearer {self.api_key}", "Content-Type": "application/json"}, json={"inputs": prompt_text, "parameters": {"max_new_tokens": 200, "temperature": 0.7, "repetition_penalty": 1.2, "return_full_text": False, "num_return_sequences": 1, "do_sample": True}}, timeout=timeout)
                response.raise_for_status()
...
def process_single_row(idx, question, lm, prompt_template):
    try:
...
def generate_openrouter_responses(csv_path: str, output_path: str, question_col: str = "question", max_workers: int = 10, progress_interval: int = 50):
    load_dotenv()
    ...
    df = prepare_for_english_generation(pd.read_csv(csv_path))
    ...
    for model_short, (model_full, provider) in models.items():
        ...
        for run in range(1, 4):
            ...
            try:
                with ThreadPoolExecutor(max_workers=max_workers) as executor:
                    futures = {executor.submit(process_single_row, idx, df.loc[idx, question_col], lm, SAKHI_PROMPT): idx for idx in rows_to_process}
        ...
        df_out.to_csv(output_path, index=False)
        print(f"\n✓ Completed all runs for {model_short}")
    print(f"\n{'='*60}")
    print("OpenRouter + Cohere Generation Complete!")
    print(f"{'='*60}")
    return df_out

def generate_huggingface_responses(csv_path: str, output_path: str, question_col: str = "question", max_workers: int = 3, progress_interval: int = 25):
    load_dotenv()
    ...
    for model_short, (model_name, endpoint_url) in models.items():
        print(f"\n{'='*60}")
        print(f"Starting model: {model_short} ({model_name})")
        print(f"{'='*60}")
        if not endpoint_url:
        ...
        for run in range(1, 4):
            ...
            try:
                for i, idx in enumerate(rows_to_process):
                    ...
                    idx_result, clean_resp, error = process_single_row(idx, question, lm, SAKHI_PROMPT)
                    if error:
                        print(f"      ✗ Row {idx}: {error}")
                        failed += 1
                    elif not clean_resp:
        ...
        print(f"\n✓ Completed all runs for {model_short}")
    print(f"\n{'='*60}")
    print("HuggingFace Generation Complete!")
    print(f"{'='*60}")
    return df_out
...
if __name__ == "__main__":
    ...
    english_jobs = [
        (SAKHI_NON_EXPERT_RAW_230, LLM_ENGLISH_NON_EXPERT_230, "230-row non-expert (EN)"),
        (SAKHI_EXPERT_RAW_150, LLM_ENGLISH_EXPERT_150, "150-row expert (EN)"),
    ]

    for csv_in, csv_out, label in english_jobs:
        if not csv_in.exists():
            print(f"⚠️ Skipping {label}: missing input {csv_in}")
            continue
        print(f"\n{'#' * 80}\n# English generation: {label}\n{'#' * 80}")
        out_s = str(csv_out)
        generate_openrouter_responses(str(csv_in), out_s, question_col="question", max_workers=10, progress_interval=50)
        generate_huggingface_responses(str(csv_in), out_s, question_col="question", max_workers=10, progress_interval=25)